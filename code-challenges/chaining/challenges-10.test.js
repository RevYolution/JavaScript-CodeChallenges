...
const count = (target, input) => {
  let counter = 0;
  let targetNumCount = input.reduce((total, numberSet) => {
...
const totalSum = (input) => {
  let arrayTotal = input.reduce((total, numberSet) => {
      let singleArrayTotal = numberSet.reduce((arrSum, num) => arrSum += num, 0)
      return total += singleArrayTotal;
  }, 0)
  return arrayTotal;
};
...
// let regExpression = /[,]/g;
// let characters = data.reduce((name, character) => {
//     if(character.gender === 'male' || character.gender === 'female'){
//         name.push(character.name)
//     }
//     return characters;
// }, []);
// return characters.toString().replace(regExpression, ' and ');
