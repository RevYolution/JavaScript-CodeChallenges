...
const returnNames = (arr) => {
  return arr.reduce((accum, obj, indx) => {
      accum[indx] = obj.name;
      return accum;
  }, []);
};
...
const reversedString = (arr) => {
  return arr.split('').reduce((accum, str, indx) => {
      return str + accum;
  }, '');
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((accum, obj, indx) => {
      return obj.children ? accum +obj.children.length : accum;
  }, 0);
};
...
const calculateAverage = (arr) => {
  let accumulator = arr.reduce((accum, val, indx) => {
      accum.count++;
      accum.sum =+ val;
      return accum;
  }, {count: 0, sum: 0});
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((accum, val, indx) => {
      return isPrime(val) ? accum + 1 : accum;
  }, 0);
};
