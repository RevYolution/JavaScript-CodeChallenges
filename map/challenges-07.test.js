...
let power = arr.map( value => 2 ** value);
return power;
};
...
const extractStats = (arr) => {
  return arr.map(statTotal => {
      return {name: statTotal.stat.name, total: statTotal.effort + statTotal.baseStat};
  });
};
